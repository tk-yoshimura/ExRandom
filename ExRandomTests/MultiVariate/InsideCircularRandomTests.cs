...
namespace ExRandom.MultiVariate.Tests {
    [TestClass()]
    public class InsideCircularRandomTests {
        [TestMethod()]
        public void InsideCircularRandomTest() {
            const int times = 10000;
            const decimal minX = -1.05m, maxX = 1.05m, minY = -1.05m, maxY = 1.05m;

            MT19937 mt = new();
            InsideCircularRandom rd = new InsideCircularRandom(mt);
            ...
            graph.Save(Workspace.OutDir + "plot_multi_inside_circular_xy.png");
        }
    }
}