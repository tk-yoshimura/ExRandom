...
namespace ExRandom.Continuous.Tests {
    [TestClass()]
    public class WignerSemicircleRandomTests {
        [TestMethod()]
        public void WignerSemicircleRandomTest() {
            const int N = 1000000, X_MIN = -2, X_MAX = 2, X_SCALE = 100;

            MT19937 mt = new();
            Random rd = new WignerSemicircleRandom(mt, s: 1);
            ...
            pg.DrawYScale(Color.Black, 0, 1m, 0.5m);

            pg.DrawLineGraph(Color.Black, X_MIN, X_MAX, cnt, 2);

            pg.DrawLine(Color.Gray, ave, 0, ave, 1);

            pg.Save(Workspace.OutDir + "plot_con_wigner_semicircle.png");
        }
    }
}