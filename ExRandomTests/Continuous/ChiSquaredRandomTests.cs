...
namespace ExRandom.Continuous.Tests {
    [TestClass()]
    public class ChiSquaredRandomTests {
        [TestMethod()]
        public void ChiSquaredRandomTest() {
            const int N = 1000000, X_MIN = -1, X_MAX = 9, X_SCALE = 10;

            MT19937 mt = new();
            Random rd = new ChiSquaredRandom(mt, k: 3);
            ...
            pg.DrawYScale(Color.Black, 0, 0.3m, 0.05m);

            pg.DrawLineGraph(Color.Black, X_MIN, X_MAX, cnt, 2);

            pg.DrawLine(Color.Gray, ave, 0, ave, 1);

            pg.Save(Workspace.OutDir + "plot_con_chisquared.png");
        }
    }
}