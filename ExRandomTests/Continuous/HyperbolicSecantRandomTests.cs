...
namespace ExRandom.Continuous.Tests {
    [TestClass()]
    public class HyperbolicSecantRandomTests {
        [TestMethod()]
        public void HyperbolicSecantRandomTest() {
            const int N = 1000000, X_MIN = -5, X_MAX = 5, X_SCALE = 10;

            MT19937 mt = new();
            Random rd = new HyperbolicSecantRandom(mt);
            ...
            pg.DrawYScale(Color.Black, 0, 0.6m, 0.1m);

            pg.DrawLineGraph(Color.Black, X_MIN, X_MAX, cnt, 2);

            pg.DrawLine(Color.Gray, ave, 0, ave, 1);

            pg.Save(Workspace.OutDir + "plot_con_hyperbolic_secant.png");
        }
    }
}